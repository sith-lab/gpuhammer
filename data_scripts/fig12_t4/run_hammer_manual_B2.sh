...
mkdir -p $HAMMER_ROOT/results/fig12_t4/B2
for model in alexnet vgg resnet dense inception; do
    echo "Processing $model"

    >  "$HAMMER_ROOT/results/fig12_t4/B2/${model}.txt"
    shuf --random-source=<(yes 42) -i 0-7184 -n 50 | while read num; do

        mem_size=$((10268835328 + (256 * $num)))
        printf "$num: ***********************\n"
        ...
        sleep 2
        nohup $HAMMER_ROOT/src/out/build/hammer_manual_agg_right $rowset_file $((num_agg - 1)) $addr_step $iterations $min_rowid $max_rowid $row_step $mem_size $num_warp $num_thread $delay 1 $count_iter >/dev/null 2>&1 &
        sleep 5

...
mkdir -p $HAMMER_ROOT/results/fig12_t4/D3
for model in alexnet vgg resnet dense inception; do
    echo "Processing $model"

    >  "$HAMMER_ROOT/results/fig12_t4/D3/${model}.txt"
    shuf --random-source=<(yes 42) -i 0-8039 -n 50 | while read num; do

        mem_size=$((22412042752 + (256 * $num)))
        printf "$num: ***********************\n"
